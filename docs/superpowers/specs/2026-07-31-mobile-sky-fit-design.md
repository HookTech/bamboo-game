...
- 不改编云/星刷点与漂移范围。
- 不改地面植被、刘海/安全区专属 inset（仅按可见宽缩字+贴边）。
- 不改 `ResolutionPolicy`、FOV、相机距离、玩法数值。

## 补充：HUD 小屏字号与边距（已确认）

`FIXED_HEIGHT` 下竖屏可见宽 ≪ 800，原先左上统计按 `DESIGN_W=800` 定位会贴边/出界。

- `uiW = view.getVisibleSize().width`
- `fontScale = clamp(uiW / DESIGN_W, 0.55, 1)`
- 左/右 X 按 `±uiW/2 + pad`；统计字号 × `fontScale`；连击条右缘跟 `uiW`
- 宽变化时在 `refresh` → `syncLayout` 重算
...
1. Creator 预览：默认窗口日月在右上可见；左上统计完整可读。
2. 缩成竖屏/窄窗口：日月仍在画面内偏右上；统计字变小且不裁切。
3. 拉回横屏：日月外移仍可见；字号恢复接近设计值。
4. 昼夜切换、光晕表现不受影响。
...
- 云仍可能在极窄屏出界；不在本任务范围。
- HUD 未做 notch safe-area；若真机刘海遮挡再加平台 inset。
